...
| **ActivityLogService** | `Info()`, `Warn()`, `Error()` → writes to ActivityLog table with action, logger class name, level, message, username. Convention: **state changes and auth events are logged, reads never are.** Each call does its own `SaveChangesAsync`, so it is a separate round-trip — keep it off high-volume paths (notification mark-as-read is deliberately unlogged) |
